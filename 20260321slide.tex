...
\begin{columns}
	...
	\begin{column}[t]{0.8\textwidth}
		\begin{description}
			...
			\item[Fun] Playing tuba in the Tokyo Gas Wind Orchestra
		\end{description}
	\end{column}
	
	\begin{column}[T]{0.17\textwidth}
		\includegraphics[width=2cm]{hayao.jpg}
	\end{column}
...
\begin{columns}
	...
	\begin{column}[t]{0.8\textwidth}
		\begin{itemize}
			\item The largest natural gas utility in Japan
	...
	\begin{column}[T]{0.17\textwidth}
		\includegraphics[width=2cm]{building.jpg}
	\end{column}
...
\begin{itemize}
\item \structure{Tomorrow graphlib, Let Us Use Everybody}(PyCon JP 2025)
...
\item \structure{Do You Know cmath Module?}(PyCon mini Shizuoka 2020)
...
%\includemedia[
%  addresource=voice/slide04.wav,
%  flashvars={
%    source=voice/slide04.wav
%   &autoPlay=true
%  },
%  transparent
%]{\color{blue}{\tiny\fbox{Play}}}{APlayer.swf}
...
\begin{center}
\LARGE{Let's implement \structure{useless} Python objects}
\end{center}
%\includemedia[
...
\begin{frame}[fragile,shrink=4]\frametitle{\texttt{for} and \texttt{Iterable}}
...
\begin{frame}[fragile,shrink=4]\frametitle{\texttt{Sequence}}
    ...
    def __getitem__(self, key):
        match key:
            ...
            case slice():
                n = len(self._data)
                s = slice(
                    key.start % n, key.stop % n, key.step
                )
                return self._data[s]
            case _:
                raise TypeError
